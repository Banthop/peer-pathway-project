#!/usr/bin/env node
/**
 * Re-send confirmation emails to buyers who were tagged confirmation_sent
 * but never received the email (no email_delivered tag).
 * 
 * Uses the Broadcast API (Marketing Pro - unlimited, bypasses transactional quota).
 * 
 * Usage:
 *   node scripts/resend-missing-confirmations.mjs              # Dry run
 *   node scripts/resend-missing-confirmations.mjs --send       # Actually send
 */
...
// ═══════════════════════════════════════════════════════════════
// EMAIL TEMPLATES (same as confirmation emails)
// ═══════════════════════════════════════════════════════════════
...
async function main() {
  console.log('\n📧 Re-send Missing Confirmation Emails (Broadcast API)\n');
  console.log(LIVE_SEND ? '🔴 LIVE MODE - broadcasts WILL be sent\n' : '🟡 DRY RUN - no emails will be sent (use --send to go live)\n');

  // Find all stripe_customer contacts who have confirmation_sent but NOT email_delivered
  ...
  const buyers = allContacts.filter(c => {
    const tags = c.tags || [];
    return tags.includes('stripe_customer') &&
           tags.includes('confirmation_sent') &&
           !tags.includes('email_delivered') &&
           !tags.includes('email_bounced'); // skip bounced - bad email address
  });

  console.log(`📋 Found ${buyers.length} buyers who paid but never received their confirmation email\n`);

  if (buyers.length === 0) {
    console.log('✅ All buyers have received their emails! Nothing to do.');
    return;
  }
  ...
  console.log(`📦 Bundle buyers to re-send: ${bundleBuyers.length}`);
  for (const c of bundleBuyers) console.log(`   ${(c.first_name + ' ' + (c.last_name || '')).trim() || 'Unknown'} <${c.email}>`);
  
  console.log(`🎥 Webinar-only buyers to re-send: ${webinarOnlyBuyers.length}`);
  for (const c of webinarOnlyBuyers) console.log(`   ${(c.first_name + ' ' + (c.last_name || '')).trim() || 'Unknown'} <${c.email}>`);
  console.log('');

  if (!LIVE_SEND) {
    console.log('💡 Run with --send to actually send these emails via Broadcast API.\n');
    return;
  }

  // Build templates with broadcast personalization
  ...
  if (bundleBuyers.length > 0) {
    console.log('\n📦 Re-sending BUNDLE confirmations via Broadcast API...');
    const result = await sendBroadcast({
      name: 'Bundle Confirmation Resend',
      from: FROM_EMAIL,
      subject: "you're in - here's everything you need",
      html: bundleHtml,
    ...
    for (const c of bundleBuyers) {
      const newTags = [...new Set([...(c.tags || []), 'resent_via_broadcast'])];
  ...
  if (webinarOnlyBuyers.length > 0) {
    console.log('\n🎥 Re-sending WEBINAR-ONLY confirmations via Broadcast API...');
    const result = await sendBroadcast({
      name: 'Webinar Confirmation Resend',
      from: FROM_EMAIL,
      subject: "you're in - here's your zoom link",
      html: webinarHtml,
  ...
  console.log(`\n✅ Done! Re-sent confirmations to ${buyers.length} buyers via Broadcast API (Marketing Pro).\n`);
}
