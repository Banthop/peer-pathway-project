...
import {
    coachConversations as initialConversations,
} from "@/data/coachDashboardData";
import type { CoachConversation } from "@/data/coachDashboardData";

export default function CoachMessages() {
    const [localConvos, setLocalConvos] = useState<CoachConversation[]>(initialConversations);
    const [activeConvo, setActiveConvo] = useState<CoachConversation>(initialConversations[0]);
    ...
    const selectConvo = (convo: CoachConversation) => {
        // Clear unread when selecting
        const updated = localConvos.map((c) =>
            c.id === convo.id ? { ...c, unread: 0 } : c,
        );
        setLocalConvos(updated);
        setActiveConvo(updated.find((c) => c.id === convo.id)!);
    };
            ...
            <div className="w-[340px] flex-shrink-0 border-r border-border flex flex-col bg-background">
                <div className="px-6 py-5 border-b border-border">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                        {localConvos.reduce((s, c) => s + c.unread, 0)} unread
                    </p>
                </div>
